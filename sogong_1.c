...
void viewAllContacts()
{
	...
	for(i=1; i<contactCount; i++)
	{
		...
		{
			if(strcmp(p2->sContact->lastName, p3->sContact->lastName) < 0 && strcmp(p2->sContact->firstName, p3->sContact->firstName) < 0)	//문자열 비교(오름차순)
			{
				p2->next = p3->next;
				p3->next = p2;
				p1->next = p3;
...
void modifyAddress(){
	...
	switch(CheckedNum) {
		...
		case '6':
		printf("6. 주소 : "); gets("%s", pContact->address); break;
		case '7':
